...
public abstract class RandomWalkComputation<E extends Writable> extends
		BasicComputation<Text, DoubleWritable, E, DoubleWritable> {
	 ...
	 */
	protected abstract double transitionProbability(Vertex<Text, DoubleWritable, E> vertex,
			double stateProbability, Edge<Text, E> edge);
	 ...
	 */
	protected abstract double recompute(Vertex<Text, DoubleWritable, E> vertex,
	...
	@Override
	public void compute(Vertex<Text, DoubleWritable, E> vertex, Iterable<DoubleWritable> messages)
			throws IOException {
		...
		if (getSuperstep() < maxSupersteps()) {
			for (Edge<Text, E> edge : vertex.getEdges()) {
